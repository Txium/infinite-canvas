...
func CreateRechargeOrder(user model.AuthUser, amountCents int, paymentMethod, paymentNote string) (model.RechargePayment, error) {
	if amountCents < 100 {
		return model.RechargePayment{}, safeMessageError{message: "充值金额不能低于 1 元"}
	}
	if amountCents > 10000000 {
		return model.RechargePayment{}, safeMessageError{message: "充值金额超过限制"}
	}
	paymentMethod = strings.TrimSpace(paymentMethod)
	if paymentMethod != "alipay" {
		return model.RechargePayment{}, safeMessageError{message: "目前仅支持支付宝"}
	}
	if !config.DatabasePersistent() {
		return model.RechargePayment{}, safeMessageError{message: "平台数据库尚未启用持久化，为保护资金安全暂不能充值"}
	}
	if !config.PaymentConfigured() {
		return model.RechargePayment{}, safeMessageError{message: "在线支付尚未配置，请管理员先配置支付商户"}
	}
	...
	if err != nil {
		return model.RechargePayment{}, err
	}
...
func CompleteEpayRecharge(values url.Values) error {
	if strings.TrimSpace(config.Cfg.EpayMerchantKey) == "" || values.Get("pid") != config.Cfg.EpayMerchantID {
		return safeMessageError{message: "支付商户配置无效"}
	}
	...
	if len(provided) != len(expected) || subtle.ConstantTimeCompare([]byte(provided), []byte(expected)) != 1 {
		return safeMessageError{message: "支付回调签名无效"}
	}
	...
	_, err = repository.CompleteRechargeOrder(order.ID, values.Get("trade_no"), "epay", values.Get("pid"), now())
	return err
}
...
func ReviewRechargeOrder(id string, status model.RechargeOrderStatus, adminID, remark string) (model.RechargeOrder, error) {
	if status != model.RechargeOrderApproved && status != model.RechargeOrderRejected {
		return model.RechargeOrder{}, safeMessageError{message: "审核状态无效"}
	}
