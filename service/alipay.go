...
func createOfficialAlipayPayURL(order model.RechargeOrder) (string, error) {
	...
	if err != nil { return "", safeMessageError{message: "支付宝应用私钥格式无效"} }
	values := url.Values{}
...
func CompleteOfficialAlipayRecharge(values url.Values) error {
	if !config.AlipayConfigured() { return safeMessageError{message: "支付宝商户配置无效"} }
	if values.Get("app_id") != config.Cfg.AlipayAppID { return safeMessageError{message: "支付宝应用不匹配"} }
	status := values.Get("trade_status")
	if status != "TRADE_SUCCESS" && status != "TRADE_FINISHED" { return safeMessageError{message: "支付尚未成功"} }
	if err := verifyAlipayRSA2(values, config.Cfg.AlipayPublicKey); err != nil { return safeMessageError{message: "支付宝回调签名无效"} }
...
func verifyAlipayRSA2(values url.Values, publicKeyText string) error {
	...
	for name := range values {
		if name != "sign" && name != "sign_type" && values.Get(name) != "" { params[name] = values.Get(name) }
	}
...
func canonicalAlipayParams(params map[string]string) string {
	...
	for key, value := range params { if key != "sign" && value != "" { keys = append(keys, key) } }
	sort.Strings(keys)
	parts := make([]string, 0, len(keys))
	for _, key := range keys { parts = append(parts, key+"="+params[key]) }
...
func parseRSAPrivateKey(value string) (*rsa.PrivateKey, error) {
	...
	if block == nil { return nil, fmt.Errorf("invalid private key PEM") }
...
func parseRSAPublicKey(value string) (*rsa.PublicKey, error) {
	block, _ := pem.Decode(normalizedPEM(value))
	if block == nil { return nil, fmt.Errorf("invalid public key PEM") }
	if key, err := x509.ParsePKIXPublicKey(block.Bytes); err == nil {
		if rsaKey, ok := key.(*rsa.PublicKey); ok { return rsaKey, nil }
	}
