...
export function CanvasMediaWorkbench({node, onClose, onOutput}:{node:CanvasNodeData; onClose:()=>void; onOutput:(blob:Blob, action:MediaAction, time:number, continueVideo:boolean, metadata:MediaProbe)=>Promise<void>}) {
    ...
    async function run(action:MediaAction, continueVideo=false) {
        if(!file || !meta) return;
        setBusy(true);setError("");
        try {const blob=await processOriginalMedia(file,action,action==="clip"?start:time,end,precise).then(r=>r.blob()); await onOutput(blob,action,time,continueVideo,meta);}
    ...
    return <Modal open title="视频处理 · 原文件" width={880} footer={null} onCancel={busy?undefined:onClose} closable={!busy} maskClosable={!busy}>
        <div className="space-y-3">
            ...
            {meta && <>
                ...
                <div className="font-mono">{timecode} · 第 {frameIndex+1} 帧 · {meta.fps.toFixed(3)} fps · {meta.width}×{meta.height} · {meta.codec}</div>
                ...
                <div className="flex flex-wrap gap-2">
                    ...
                    <Button disabled={busy||!meta.audioCodec} onClick={()=>void run("audio")}>提取音频</Button><Button disabled={busy||!meta.subtitleTracks} onClick={()=>void run("subtitles")}>移除独立字幕轨</Button>
                </div>
