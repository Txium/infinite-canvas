...
export function CanvasAudioAIPanel({characterId,characterName,onClose,onBind}:{characterId?:string;characterName:string;onClose:()=>void;onBind:(id:string)=>void}) {
 const [capabilities,setCapabilities]=useState<MediaCapability[]>([]);
 const [profiles,setProfiles]=useState<VoiceProfile[]>([]);
 const [profile,setProfile]=useState<VoiceProfile>(()=>({character_id:characterId||`char_${nanoid()}`,character_name:characterName,reference_images:[],voice_provider:"",voice_id:"",voice_prompt:"",default_speed:1,default_emotion:"",default_style:""}));
 ...
 async function save(){setBusy(true);try{const saved=await saveVoiceProfile(profile);setProfiles(p=>[saved,...p.filter(v=>v.character_id!==saved.character_id)]);onBind(saved.character_id);setStatus("角色档案已保存并绑定当前节点；其他节点可选择同一角色。不调用模型。");}catch(e){setStatus(e instanceof Error?e.message:"保存失败");}finally{setBusy(false);}}
 return <Modal open title="角色音色与媒体AI" footer={null} onCancel={onClose} width={720}>
  ...
  <p className="mb-2 text-xs opacity-70">角色使用独立稳定ID，不等于节点ID。选择已有角色再保存，可让不同图片/视频/音频节点绑定同一档案。这里只保存资料，不调用模型。不要填写API Key。</p>
  ...
  <label className="block mt-3">角色参考图地址（每行一张，最多8张）<textarea className="block w-full border border-current rounded bg-transparent p-2" rows={2} value={(profile.reference_images||[]).join("\n")} onChange={e=>setProfile(p=>({...p,reference_images:e.target.value.split("\n").filter(Boolean)}))}/></label>
