...
export default function AdminGenerationTasksPage() {
    ...
    const columns: ProColumns<AdminGenerationTask>[] = [
        ...
        {title:"任务状态",dataIndex:"status",width:150,valueType:"select",valueEnum:{queued:{text:"排队"},processing:{text:"处理中"},reconciling:{text:"等待上游确认"},timed_out_unknown:{text:"超时待确认"},completed:{text:"成功"},failed:{text:"失败"}},render:(_,item)=><Tag color={item.status==="completed"?"success":item.status==="failed"?"error":item.status==="reconciling"||item.status==="timed_out_unknown"?"warning":"processing"}>{item.status==="reconciling"||item.status==="timed_out_unknown"?"等待上游确认":item.status}</Tag>},
