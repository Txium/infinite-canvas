...
export default function AdminGenerationTasksPage() {
    ...
    const columns: ProColumns<AdminGenerationTask>[] = [
		...
		{title:"操作",valueType:"option",width:110,fixed:"right",render:(_,item)=>item.kind==="image"&&Boolean(item.upstreamTaskId)&&["processing","reconciling","timed_out_unknown"].includes(item.status)?<Button size="small" loading={reconcilingTaskId===item.id} onClick={async()=>{try{setReconcilingTaskId(item.id);await reconcileAdminGenerationTask(token,item.id);message.success("已查询原上游任务，没有重新提交");actionRef.current?.reload();}catch(error){message.error(error instanceof Error?error.message:"任务对账失败");}finally{setReconcilingTaskId("");}}}>立即对账</Button>:"-"},
