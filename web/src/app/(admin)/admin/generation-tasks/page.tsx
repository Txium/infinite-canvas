...
import { fetchAdminGenerationTasks, importAdminGenerationTask, reconcileAdminGenerationTask, type AdminGenerationTask } from "@/services/api/admin";
...
export default function AdminGenerationTasksPage() {
	...
	const [reconcilingTaskId,setReconcilingTaskId] = useState("");
    ...
    const columns: ProColumns<AdminGenerationTask>[] = [
		...
		{title:"中转站 / Adapter",dataIndex:"channelName",width:180,ellipsis:true,search:false,render:(_,item)=><Space direction="vertical" size={0}><span>{item.channelName||item.provider||"-"}</span><Typography.Text type="secondary">{item.adapter||"-"}</Typography.Text></Space>},
		{title:"上游模型 / Endpoint",dataIndex:"upstreamModelId",width:220,ellipsis:true,search:false,render:(_,item)=><Space direction="vertical" size={0}><Typography.Text>{item.upstreamModelId||"-"}</Typography.Text><Typography.Text type="secondary">{item.providerEndpoint||"-"}</Typography.Text></Space>},
        ...
        {title:"任务状态",dataIndex:"status",width:170,valueType:"select",valueEnum:{queued:{text:"排队"},processing:{text:"处理中"},reconciling:{text:"等待上游确认"},timed_out_unknown:{text:"超时待确认"},completed:{text:"成功"},failed:{text:"失败"}},render:(_,item)=><Space direction="vertical" size={0}><Tag color={item.status==="completed"?"success":item.status==="failed"?"error":item.status==="reconciling"||item.status==="timed_out_unknown"?"warning":"processing"}>{item.status==="reconciling"||item.status==="timed_out_unknown"?"等待上游确认":item.status}</Tag>{item.providerTaskStatus?<Typography.Text type="secondary">上游：{item.providerTaskStatus}</Typography.Text>:null}</Space>},
		{title:"请求",dataIndex:"upstreamRequestSent",width:120,search:false,render:(_,item)=><Tag color={item.upstreamRequestSent?"success":"error"}>{item.upstreamRequestSent?`已发送${item.upstreamHttpStatus?` ${item.upstreamHttpStatus}`:""}`:"未发送"}</Tag>},
		{title:"分辨率链路",dataIndex:"frontendSelectedResolution",width:200,search:false,render:(_,item)=><Typography.Text>{[item.frontendSelectedResolution,item.providerRequestedResolution,item.providerFinalResolution].map((value)=>value||"-").join(" → ")}</Typography.Text>},
        ...
        {title:"错误",dataIndex:"error",ellipsis:true,render:(_,item)=><Space direction="vertical" size={0}><Typography.Text type={item.error?"danger":"secondary"}>{item.error||"-"}</Typography.Text>{item.errorCode?<Tag color="error">{item.errorCode}</Tag>:null}</Space>},
		...
		{title:"操作",valueType:"option",width:110,fixed:"right",render:(_,item)=>item.kind==="image"&&Boolean(item.upstreamTaskId)&&["processing","reconciling","timed_out_unknown","failed"].includes(item.status)?<Button size="small" loading={reconcilingTaskId===item.id} onClick={async()=>{try{setReconcilingTaskId(item.id);await reconcileAdminGenerationTask(token,item.id);message.success("已查询原上游任务，没有重新提交");actionRef.current?.reload();}catch(error){message.error(error instanceof Error?error.message:"任务对账失败");}finally{setReconcilingTaskId("");}}}>立即对账</Button>:"-"},
