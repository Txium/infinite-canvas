...
export default function AdminFinancePage() {
	...
	const expensePending = useRef(false);
    ...
    const load = async () => {
		...
		if (!token) return;
		try {
			const [finance, runtime, expenseItems] = await Promise.all([fetchAdminFinanceSummary(token, period), fetchAdminRuntimeReadiness(token), fetchAdminOperatingExpenses(token)]);
			if (sequence !== loadSequence.current || useUserStore.getState().token !== token) return;
			setSummary(finance); setReadiness(runtime); setExpenses(expenseItems || []);
		} catch {
			if (sequence === loadSequence.current && useUserStore.getState().token === token) void message.error("财务数据读取失败，请刷新重试");
		}
	};
    useEffect(() => { setSummary(null); setReadiness(null); setExpenses([]); void load(); return () => { loadSequence.current++; }; }, [token, period]);
    ...
    return <main className="p-6"><Space direction="vertical" size={16} className="w-full">
        ...
        {summary && !summary.upstreamCostReady ? <Alert type="warning" showIcon message="上游成本尚未全部核实" description="部分成功任务成本缺失或仍使用配置估算；下方成本、毛差和净收益均为待核账数据，不能作为真实利润。" /> : null}
        ...
        <Row gutter={[16,16]}>
            ...
            <Col xs={24} md={12} xl={6}><Card><Statistic title="所选期间生成收入" value={formatCNY(summary?.selected?.revenueCents || 0)} /></Card></Col>
            <Col xs={24} md={12} xl={6}><Card><Statistic title="已记录上游成本（含估算）" value={formatCNY(summary?.selectedProviderCostCents || 0)} /></Card></Col>
            <Col xs={24} md={12} xl={6}><Card><Statistic title="当前上游成本准备金" value={formatCNY(summary?.providerReserveCents || 0)} /></Card></Col>
            <Col xs={24} md={12} xl={6}><Card><Statistic title="估算毛差" value={formatCNY(summary?.selectedGrossProfitCents || 0)} /></Card></Col>
		...
		<Card title="中转站已记录成本（含估算）"><Table rowKey="provider" size="small" pagination={false} dataSource={summary?.providerCosts || []} columns={[{title:"中转站",dataIndex:"provider",render:(value:string)=>({"302":"302.AI",wavespeed:"WaveSpeed",lec:"LEC",seedance_nz:"seedance.nz"}[value]||value)},{title:"今日成本",render:(_:unknown,item:ProviderCostSummary)=>formatCNY(item.todayCents)},{title:"近7日成本",render:(_:unknown,item:ProviderCostSummary)=>formatCNY(item.last7DaysCents)},{title:"累计成本",render:(_:unknown,item:ProviderCostSummary)=>formatCNY(item.allTimeCents)}]} /></Card>
        ...
        <Row gutter={[16,16]}>
            ...
            <Col xs={24} md={12} xl={6}><Card><Statistic title="已记录上游成本（含估算）" value={formatCNY(summary?.actualProviderCostCents || 0)} /></Card></Col>
            <Col xs={24} md={12} xl={6}><Card><Statistic title="上游成本准备金" value={formatCNY(summary?.providerReserveCents || 0)} /></Card></Col>
            <Col xs={24} md={12} xl={6}><Card><Statistic title="估算模型毛差" value={formatCNY(summary?.grossProfitCents || 0)} /></Card></Col>
        ...
        <Modal title="登记运营费用" open={expenseOpen} confirmLoading={expenseLoading} cancelButtonProps={{ disabled: expenseLoading }} closable={!expenseLoading} maskClosable={!expenseLoading} keyboard={!expenseLoading} onCancel={() => { if (!expensePending.current) setExpenseOpen(false); }} onOk={async () => {
			if (expensePending.current) return;
			expensePending.current = true; setExpenseLoading(true);
			try {
				const values = await expenseForm.validateFields();
				await recordAdminOperatingExpense(token, { category: values.category, amountCents: Math.round(values.amountYuan * 100), date: values.date?.format("YYYY-MM-DD"), reason: values.reason, reference: values.reference });
				void message.success("运营费用已登记"); setExpenseOpen(false); expenseForm.resetFields(); await load();
			} catch (error) {
				if (!(error && typeof error === "object" && "errorFields" in error)) void message.error(error instanceof Error ? error.message : "登记失败，请先核对费用列表再重试");
			} finally { expensePending.current = false; setExpenseLoading(false); }
		}}>
