...
export type ProviderBalanceSnapshot = { upstreamBalanceAmount?: string; upstreamBalanceCurrency?: string; upstreamBalanceCheckedAt?: string; upstreamBalanceAttemptedAt?: string; upstreamBalanceError?: string };
export type AdminModelProvider = ProviderBalanceSnapshot & { id: string; name: string; code: string; baseUrl: string; hasApiKey: boolean; enabled: boolean; priority: number; timeout: number; balanceCents: number | null; balanceCheckedAt: string; warningBalanceCents: number; criticalBalanceCents: number; lowBalanceCents: number; ready: boolean; routeCount: number; enabledRouteCount: number; balanceStatus: "disabled" | "not_ready" | "unknown" | "very_low" | "critical" | "warning" | "normal"; balanceMessage: string; remark: string };
export type AdminModelRoute = { id: string; modelId: string; variantId: string; providerId: string; upstreamModelId: string; protocol: string; adapter: string; endpoint: string; priority: number; enabled: boolean; catalogStatus?: "available" | "missing"; catalogCheckedAt?: string; catalogAutoDisabled?: boolean };
export type AdminModelVariant = { id: string; modelId: string; name: string; providerCode: string; upstreamModelId: string; costCents?: number; costText: string; priceCents?: number; priceText: string; billingUnit: string; pricingMode: "fixed" | "dynamic" | "disabled"; priceFormula: string; marginText: string; personNote: string; refundPolicy: string; sourceUrl: string; remark: string; verificationStatus: "VERIFIED" | "TESTING" | "UNVERIFIED"; enabled: boolean; sort: number };
...
export type AdminModelProviderTest = { ok: boolean; providerId: string; message: string; balanceText?: string; balanceAmount?: string; balanceCurrency?: string; balanceCheckedAt?: string; models?: string[] };
...
export type AdminProviderCatalogSync = { providerId: string; providerCode: string; upstreamModels: number; checkedRoutes: number; disabledRoutes: string[]; checkedAt: string };
export async function syncAdminModelProviderCatalog(token: string, id: string) { return apiPost<AdminProviderCatalogSync>(`/api/admin/model-providers/${encodeURIComponent(id)}/sync-models`, {}, token); }
...
export type FinancePeriodSummary = { rechargeCents: number; refundCents: number; netRechargeCents: number; revenueCents: number; releasedCents: number; settledTasks: number; releasedTasks: number };
...
export type AdminGenerationTask = { id: string; userId: string; userDisplayName: string; kind: "image" | "video" | "audio"; model: string; status: string; billingStatus: string; priceCents: number; source: string; channelName: string; provider: string; upstreamModelId: string; adapter: string; providerEndpoint: string; upstreamRequestSent: boolean; upstreamHttpStatus: number; upstreamTaskId: string; providerTaskStatus: string; frontendSelectedResolution: string; providerRequestedResolution: string; providerFinalResolution: string; providerOriginalResultUrl: string; canvasResultUrl: string; errorCode: string; resultUrl: string; error: string; createdAt: string; completedAt: string };
